...
public class DomainSocketFactory {
   ...
   */
  private static WeakHashMap<String,String> path=new WeakHashMap<String,String>();
  public PathInfo getPathInfo(InetSocketAddress addr, DFSClient.Conf conf) {
    ...
    String key=conf.getDomainSocketPath()+addr.getPort();
    String escapedPath = path.get(key);
    if(escapedPath==null)
    {
    	escapedPath=DomainSocket.getEffectivePath(
    	        conf.getDomainSocketPath(), addr.getPort());
    	path.put(key,escapedPath);
    }
