...
public class LocatedBlock {
  ...
  private static DatanodeStorageInfo[] sort(DatanodeStorageInfo[] storages)
  {
	     DatanodeStorageInfo[] storagesToAdd=new DatanodeStorageInfo[storages.length];
	      int i=0;
	      for(DatanodeStorageInfo info:storages)
	      {
	    	  if(info.getStorageType().equals(StorageType.SSD))
	    	  {
	    		  storagesToAdd[i++]=info;
	    	  }
	      }
	      for(DatanodeStorageInfo info:storages)
	      {
	      ...
	      return storagesToAdd;
  }
  public LocatedBlock(ExtendedBlock b, DatanodeStorageInfo[] storages,
      long startOffset, boolean corrupt) {
	
    this(b, DatanodeStorageInfo.toDatanodeInfos(sort(storages)),
        DatanodeStorageInfo.toStorageIDs(sort(storages)),
        DatanodeStorageInfo.toStorageTypes(sort(storages)),
