...
public class DFSUtil {
  ...
  private static Map<String, InetSocketAddress> getAddressesForNameserviceId(
    ...
    for (String nnId : emptyAsSingletonNull(nnIds)) {
      ...
      if (address != null) {
        InetSocketAddress isa =DFSInputStream.getAdress(address);
        if (isa.isUnresolved()) {
  ...
  public static Collection<URI> getNameServiceUris(Configuration conf,
    ...
    for (String key : keys) {
      ...
      if (addr != null) {
        URI uri = createUri("hdfs", DFSInputStream.getAdress(addr));
        if (!uriFound) {
          uriFound = true;
          ret.add(uri);
        } else {
  ...
  static String substituteForWildcardAddress(String configuredAddress,
    ...
    InetSocketAddress sockAddr =DFSInputStream.getAdress(configuredAddress);
    InetSocketAddress defaultSockAddr = DFSInputStream.getAdress(defaultHost
        + ":0");
  ...
  static String[] getSuffixIDs(final Configuration conf, final String addressKey,
    ...
    for (String nsId : emptyAsSingletonNull(nsIds)) {
      ...
      for (String nnId : emptyAsSingletonNull(nnIds)) {
        ...
        try {
          s = DFSInputStream.getAdress(addr);
        } catch (Exception e) {
