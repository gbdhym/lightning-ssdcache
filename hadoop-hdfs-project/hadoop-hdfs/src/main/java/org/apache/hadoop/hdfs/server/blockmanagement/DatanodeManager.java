...
public class DatanodeManager {
	 ...
	 * with this version
	 */
	private HashMap<String, Integer> datanodesSoftwareVersions = new HashMap<String, Integer>(
			4, 0.75f);
	...
	DatanodeManager(final BlockManager blockManager,
		...
		this.SSDPercent=conf.getInt("pezy.hdfs.ssd.read.percent",100);
		...
		this.rejectUnresolvedTopologyDN = conf
				.getBoolean(
		...
		final int blockInvalidateLimit = Math.max(
				20 * (int) (heartbeatIntervalSeconds),
		...
		this.checkIpHostnameInRegistration = conf
				.getBoolean(
		...
		this.timeBetweenResendingCachingDirectivesMs = conf
				.getLong(
	...
	private static long getStaleIntervalFromConf(Configuration conf,
		...
		final long minStaleInterval = conf
				.getInt(DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_MINIMUM_INTERVAL_KEY,
	...
	void activate(final Configuration conf) {
		final DecommissionManager dm = new DecommissionManager(namesystem,
				blockManager);
		this.decommissionthread = new Daemon(
				dm.new Monitor(
						conf.getInt(
	...
	public void sortLocatedBlocks(final String targethost,
		...
		Comparator<DatanodeInfo> comparator = avoidStaleDataNodesForRead ? new DFSUtil.DecomStaleComparator(
				staleInterval) : DFSUtil.DECOM_COMPARATOR;
		
		for (LocatedBlock b : locatedblocks) {
			HashMap<DatanodeInfo, StorageType> dsMap = new HashMap();
			HashMap<DatanodeInfo, String> dsIDMap = new HashMap();
			if (b.getLocations() != null)
				for (int i = 0; i < b.getLocations().length; i++) {

					dsMap.put(b.getLocations()[i], b.getStorageTypes()[i]);
					dsIDMap.put(b.getLocations()[i], b.getStorageIDs()[i]);
				}
			...
			int randV=Math.abs(rand.nextInt(100));
			
			if (randV<=this.SSDPercent&&b.getLocations() != null) {
				DatanodeInfo[] newLoc = new DatanodeInfo[b.getLocations().length];
				int i = 0;
				for (DatanodeInfo info : b.getLocations()) {
					if (dsMap.get(info) != null
							&& dsMap.get(info).equals(StorageType.SSD)) {
						newLoc[i] = info;
						try {
							b.getStorageTypes()[i] = StorageType.SSD;
							b.getStorageIDs()[i] = dsIDMap.get(info);
						} catch (Throwable e) {
							LOG.error("", e);
						}
						i++;
				...
				for (DatanodeInfo info : b.getLocations()) {
					if (dsMap.get(info) == null
						...
						newLoc[i] = info;
						try {
							b.getStorageTypes()[i] = dsMap.get(info);
							b.getStorageIDs()[i] = dsIDMap.get(info);
						} catch (Throwable e) {
							LOG.error("", e);
						}
				...
				for (int j = 0; j < b.getLocations().length; j++) {
					b.getLocations()[j] = newLoc[j];
				}
			}
		}
	}
	...
	DatanodeDescriptor getDatanodeDescriptor(String address) {
		...
		if (node == null) {
			...
			List<Node> rackNodes = getNetworkTopology().getDatanodesInRack(
					networkLocation);
			if (rackNodes != null) {
				// Try something machine local.
				for (Node rackNode : rackNodes) {
					if (((DatanodeDescriptor) rackNode).getIpAddr()
							.equals(host)) {
				...
				if (node == null && !rackNodes.isEmpty()) {
					node = (DatanodeDescriptor) (rackNodes.get(DFSUtil
							.getRandom().nextInt(rackNodes.size())));
				}
			}
			...
			// cluster.
			if (node == null) {
				node = (DatanodeDescriptor) getNetworkTopology().chooseRandom(
						NodeBase.ROOT);
			}
		}
		return node;
	...
	public void removeDatanode(final DatanodeID node)
		...
		try {
			...
			if (descriptor != null) {
				...
				NameNode.stateChangeLog.warn("BLOCK* removeDatanode: " + node
						+ " does not exist");
			}
		} finally {
		...
		synchronized (datanodeMap) {
			...
			if (d != null && isDatanodeDead(d)) {
				NameNode.stateChangeLog
						.info("BLOCK* removeDeadDatanode: lost heartbeat from "
								+ d);
				removeDatanode(d);
			}
		}
	...
	void addDatanode(final DatanodeDescriptor node) {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug(getClass().getSimpleName() + ".addDatanode: " + "node "
					+ node + " is added to datanodeMap.");
		}
	}
	...
	private void wipeDatanode(final DatanodeID node) {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug(getClass().getSimpleName() + ".wipeDatanode(" + node
					+ "): storage " + key + " is removed from datanodeMap.");
		}
	}
	...
	private String resolveNetworkLocationWithFallBackToDefaultLocation(
		...
		try {
			...
			LOG.error("Unresolved topology mapping. Using "
					+ NetworkTopology.DEFAULT_RACK + " for host "
					+ node.getHostName());
	...
	private String resolveNetworkLocation(DatanodeID node)
		...
		if (rName == null) {
			...
			throw new UnresolvedTopologyException(
					"Unresolved topology mapping for host "
							+ node.getHostName());
		} else {
	...
	private List<String> getNetworkDependencies(DatanodeInfo node)
		...
		if (dnsToSwitchMapping instanceof DNSToSwitchMappingWithDependency) {
			// Get dependencies
			dependencies = ((DNSToSwitchMappingWithDependency) dnsToSwitchMapping)
					.getDependency(node.getHostName());
	...
	private void removeDecomNodeFromList(final List<DatanodeDescriptor> nodeList) {
		...
		for (Iterator<DatanodeDescriptor> it = nodeList.iterator(); it
				.hasNext();) {
			DatanodeDescriptor node = it.next();
	...
	public void registerDatanode(DatanodeRegistration nodeReg)
		...
		if (dnAddress != null) {
			...
			if (checkIpHostnameInRegistration && !isNameResolved(dnAddress)) {
				...
				final String message = "hostname cannot be resolved (ip=" + ip
						+ ", hostname=" + hostname + ")";
		...
		try {
			...
			try {
				...
				if (this.rejectUnresolvedTopologyDN) {
					nodeDescr
							.setNetworkLocation(resolveNetworkLocation(nodeDescr));
					nodeDescr
							.setDependentHostNames(getNetworkDependencies(nodeDescr));
				} else {
					nodeDescr
							.setNetworkLocation(resolveNetworkLocationWithFallBackToDefaultLocation(nodeDescr));
					nodeDescr
							.setDependentHostNames(getNetworkDependenciesWithDefault(nodeDescr));
				}
	...
	public List<DatanodeDescriptor> getDatanodeListForReport(
		...
		final HostFileManager.HostSet includedNodes = hostFileManager
				.getIncludes();
		final HostFileManager.HostSet excludedNodes = hostFileManager
				.getExcludes();
		...
		synchronized (datanodeMap) {
			...
			for (DatanodeDescriptor dn : datanodeMap.values()) {
				...
				foundNodes.add(HostFileManager
		...
		if (listDeadNodes) {
			for (InetSocketAddress addr : includedNodes) {
				...
				DatanodeDescriptor dn = new DatanodeDescriptor(new DatanodeID(
						addr.getAddress().getHostAddress(), addr.getHostName(),
						"", addr.getPort() == 0 ? defaultXferPort
								: addr.getPort(), defaultInfoPort,
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("getDatanodeListForReport with " + "includedNodes = "
					+ hostFileManager.getIncludes() + ", excludedNodes = "
					+ hostFileManager.getExcludes() + ", foundNodes = "
			...
			synchronized (datanodeMap) {
				...
				if (blocks != null) {
					...
					for (BlockInfoUnderConstruction b : blocks) {
						final DatanodeStorageInfo[] storages = b
						...
						for (int i = 0; i < storages.length; i++) {
							if (!storages[i].getDatanodeDescriptor().isStale(
									staleInterval)) {
						...
						if (recoveryLocations.size() > 1) {
							if (recoveryLocations.size() != storages.length) {
								LOG.info("Skipped stale nodes for recovery : "
										+ (storages.length - recoveryLocations
												.size()));
							}
							brCommand
									.add(new RecoveringBlock(
											new ExtendedBlock(blockPoolId, b),
											DatanodeStorageInfo
													.toDatanodeInfos(recoveryLocations),
				...
				List<BlockTargetPair> pendingList = nodeinfo
						.getReplicationCommand(maxTransfers);
				if (pendingList != null) {
					cmds.add(new BlockCommand(DatanodeProtocol.DNA_TRANSFER,
							blockPoolId, pendingList));
				}
				// check block invalidation
				Block[] blks = nodeinfo
						.getInvalidateBlocks(blockInvalidateLimit);
				if (blks != null) {
				...
				if (shouldSendCachingCommands
						&& ((nowMs - nodeinfo
								.getLastCachingDirectiveSentTimeMs()) >= timeBetweenResendingCachingDirectivesMs)) {
