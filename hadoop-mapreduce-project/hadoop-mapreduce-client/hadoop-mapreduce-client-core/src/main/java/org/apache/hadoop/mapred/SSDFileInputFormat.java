...
public abstract class SSDFileInputFormat<K, V> implements InputFormat<K, V> {

  public static final Log LOG =
    LogFactory.getLog(SSDFileInputFormat.class);
  ...
  public InputSplit[] getSplits(JobConf job, int numSplits)
    ...
    int priorityDegree=job.getInt("pezy.hdfs.ssd.inputformat.priority.degree", 1);
    int repsn=job.getInt("pezy.hdfs.ssd.inputformat.repsn", 1);
        ...
        if (isSplitable(fs, path)) {
          ...
          while (((double) bytesRemaining)/splitSize > SPLIT_SLOP) {
            String[][] splitHosts = getSplitHostsAndCachedHosts(blkLocations,
                length-bytesRemaining, splitSize, clusterMap,priorityDegree,repsn);
          ...
          if (bytesRemaining != 0) {
            String[][] splitHosts = getSplitHostsAndCachedHosts(blkLocations, length
                - bytesRemaining, bytesRemaining, clusterMap,priorityDegree,repsn);
            splits.add(makeSplit(path, length - bytesRemaining, bytesRemaining,
                splitHosts[0], splitHosts[1]));
          }
        } else {
          String[][] splitHosts = getSplitHostsAndCachedHosts(blkLocations,0,length,clusterMap,priorityDegree,repsn);
  ...
  protected String[] getSplitHosts(BlockLocation[] blkLocations, 
      long offset, long splitSize, NetworkTopology clusterMap,int degree,int repsN) throws IOException {
    return getSplitHostsAndCachedHosts(blkLocations, offset, splitSize,
        clusterMap,degree,repsN)[0];
  }
  ...
  private String[][] getSplitHostsAndCachedHosts(BlockLocation[] blkLocations, 
      long offset, long splitSize, NetworkTopology clusterMap,int priorityDegree,int repsN)
  throws IOException {
    ...
    for (index = startIndex; index <= endIndex; index++) {
      ...
      for (String topo: allTopos) {
        ...
        nodeInfo.addValue(index, bytesInThisBlock,priority);
        parentNodeInfo.addValue(index, bytesInThisBlock,priority);
        priority+=priorityDegree;
    ...
    return new String[][] { identifyHosts(Math.min(allTopos.length,repsN), racksMap),
        new String[0]};
  }
  ...
  private static class NodeInfo {
    ...
    final HashMap<Integer,Integer>  priorityMap;
    final HashMap<Integer,Long>  sizeMap;
    int maxPriority=1;
    	...
    	else
    	{
    		synchronized(this)
    		{
    			...
    			else
    			{
    				...
    				for(int id:blockIds)
    				{
    					value+=(sizeMap.get(blockIds))/priorityMap.get(blockIds);
    				}
    				return value;
    			}
    		}
    ...
    void addValue(int blockIndex, long value,int priority) {
      if (blockIds.add(blockIndex) == true) {
        //this.value += value;
    	sizeMap.put(blockIndex, value);
        priorityMap.put(blockIndex, priority);
