...
public class TestShortCircuitLocalRead {
 ...
 static final int blockSize = 51200;
 ...
 static 
 {
	  Thread d=new Thread()
	  {
		  public void run()
		  {
			 ...
			 System.out.println("times is:"+(times-timesOld)*1000/(end-start));
			  }
 ...
 static void checkFileContent(final URI uri, final Path name, final byte[] expected,
     int readOffset, final String readingUser, final Configuration conf,
     boolean legacyShortCircuitFails)
     throws IOException, InterruptedException {
   // Ensure short circuit is enabled
	final DistributedFileSystem fs = getFileSystem(readingUser, uri, conf);
   ...
   List<Thread> ll=new ArrayList<Thread>();
   for(int i=0;i<50;i++)
   {
   	final int n=i;
   	Thread d=new Thread()
   	{
   		...
   		{
   		    try {
   		    	Configuration confn =new Configuration(conf);
   		    	confn.set(DFS_CLIENT_CONTEXT, n+"");
   		    	DistributedFileSystem fs = getFileSystem(readingUser, uri, confn);
   		    	
					FSDataInputStream stm = fs.open(name);
					long length=fs.getFileStatus(name).getLen();
					Random r=new Random();
					while(true)
					{
						stm.seek(r.nextInt((int)length));
						stm.readByte();
						times++;
						if(false)
						{
 ...
 public void doTestShortCircuitReadImpl(boolean ignoreChecksum, int size,
   ...
   String locations1 = "[disk]/opt/tempt1,[DISK]/opt/tempt2,[DISK]/opt/tempt3,[sSd]/opt/tempt4";
